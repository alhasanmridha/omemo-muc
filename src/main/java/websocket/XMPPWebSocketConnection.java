...
import jwebsocket.AbstractBody;
import jwebsocket.BOSHClient;
import jwebsocket.BOSHClientConfig;
import jwebsocket.BOSHClientConnEvent;
import jwebsocket.BOSHClientConnListener;
import jwebsocket.BOSHClientRequestListener;
import jwebsocket.BOSHClientResponseListener;
import jwebsocket.BOSHException;
import jwebsocket.BOSHMessageEvent;
import jwebsocket.BodyQName;
import jwebsocket.ComposableBody;
 ...
 */
public class XMPPWebSocketConnection extends AbstractXMPPConnection {
    private static final Logger LOGGER = Logger.getLogger(XMPPWebSocketConnection.class.getName());
     ...
     */
    public XMPPWebSocketConnection(String username, String password, boolean https, String host, int port, String filePath, DomainBareJid xmppServiceDomain) {
        this(WebSocketConfiguration.builder().setUseHttps(https).setHost(host)
     ...
     */
    public XMPPWebSocketConnection(WebSocketConfiguration config) {
        super(config);
        this.config = config;
    }
...
//                    .setNamespaceDefinition("xmpp", XMPP_BOSH_NS)
    ...
    protected void afterSaslAuthenticationSuccess()
        ...
        ComposableBody composeableBody = ComposableBody.builder().setNamespaceDefinition("xmpp",
                XMPPWebSocketConnection.XMPP_BOSH_NS).setAttribute(
                BodyQName.createWithPrefix(XMPPWebSocketConnection.XMPP_BOSH_NS, "restart",
                        "xmpp"), "true").setAttribute(
                BodyQName.create(XMPPWebSocketConnection.BOSH_URI, "to"), getXMPPServiceDomain().toString()).build();
    ...
    private class BOSHPacketReader implements BOSHClientResponseListener {
        ...
        public void responseReceived(BOSHMessageEvent event) {
            ...
            if (body != null) {
                try {
                    if (sessionID == null) {
                        sessionID = body.getAttribute(BodyQName.create(XMPPWebSocketConnection.BOSH_URI, "sid"));
                    }
                    if (streamId == null) {
                        streamId = body.getAttribute(BodyQName.create(XMPPWebSocketConnection.BOSH_URI, "authid"));
                    }
